...
function parsePaymentRequired(headerValue) {
  ...
  return {
    ...
    requirement: {
      ...
      maxAmountRequired: String(requirement.maxAmountRequired || requirement.amount),
...
export async function x402Fetch(url, body, privateKey) {
  ...
  const paymentHeader = initialResp.headers.get("X-Payment-Required") || initialResp.headers.get("payment-required")
  let parsed = null
  let bodyText = null
  ...
  if (!parsed) {
    bodyText = await initialResp.text()
